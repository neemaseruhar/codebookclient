...
export const ProductCard = ({product}) => {
    ...
    const {product_id , name, overview, poster, price, rating, best_seller} = product;
    const {addToCart,removeFromCart,cartList} = useCart();
    const [inCart,setInCart] = useState(false);

    useEffect(() => {
        const productInCart = cartList.find(item => item.id === product_id)
        if(productInCart){
            setInCart(true)
        }else{
            setInCart(false)
  ...
  return (
    <div className="m-3 max-w-sm bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700 ">
        <Link to={`/products/${product_id}`} className="relative" >
            { best_seller && <span className="absolute top-4 left-2 px-2 bg-orange-500 bg-opacity-90 text-white rounded  dark:text-white">Best Seller</span> }
        ...
        <div className="p-5">
            ...
            <p className="mb-3 font-normal text-gray-700 dark:text-white">{overview}</p>
