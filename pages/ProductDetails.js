...
export const ProductDetails = () => {
  ...
  useEffect(() => {
    const productInCart = cartList.find(item => item.product_id === product.product_id)
    if(productInCart){
        setInCart(true)
    }else{
        setInCart(false)
  ...
  return (
    <main>
        <section>
          <h1 className="mt-10 mb-5 text-4xl text-center font-bold dark:text-slate-200 text-gray-500 ">{product.name}</h1>
          <p className="mb-5 text-lg text-center text-gray-500 dark:text-slate-200">{product.overview}</p>
          <div className="flex flex-wrap justify-around">
            <div className="max-w-xl my-3">
              <img className="rounded" src={product.poster} alt={product.name} />
            </div>
            <div className="max-w-xl my-3">
              <p className="text-3xl font-bold text-gray-500 dark:text-slate-200">
              ...
              <p className="text-lg text-gray-500 dark:text-slate-200">
